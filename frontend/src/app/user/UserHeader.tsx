...
export function UserHeader({ onMenuClick }: UserHeaderProps) {
  ...
  return (
    <header className="h-16 flex items-center justify-between px-4 md:px-8 border-b border-transparent bg-bg-deep/80 backdrop-blur-md sticky top-0 z-40">
          ...
          <AnimatePresence>
            {isNotificationOpen && (
              <motion.div 
                ...
                className="fixed inset-x-4 md:inset-auto md:absolute md:-right-2 top-[72px] md:top-full mt-0 md:mt-2 md:w-80 bg-surface-dark border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-[100] md:origin-top-right flex flex-col max-h-[80vh]"
              >
                ...
                <div className="p-4 text-center text-sm text-text-dim">{t('header.noNotification')}</div>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
